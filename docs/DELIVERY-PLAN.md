...
Unlike the other new portfolio repositories, Status Bar Tasks already has substantial implementation. This plan focuses on correctness, UX alignment, integration testing, security, and packaged-release confidence rather than scaffolding a product from zero.
...
- The final README gallery uses two real 1200×800 native-alpha captures from Status Bar Tasks 0.1.0 installed in stable VS Code: the task menu over synthetic TypeScript code and the compact settings panel beside it. The 256×256 Marketplace icon is rendered directly from the approved transparent raster; no SVG reinterpretation remains.
...
15. public copy must describe this project as an independent implementation.
...
This product is an independent implementation. It carries only the project MIT `LICENSE`.

Before release:

- list third-party dependencies and licenses;
- do not add public attribution of another task-runner extension.
...
- VSIX creation;
- package contents/license/schema/media inspection;
...
#### SBT-001 — Complete license audit
Priority: P0  
Depends on: none

Verify dependency licenses and keep public copy free of third-party product attribution.
