...
The setting `tasks.statusbar.default.hide` is `false` by default. Hide all is optional. A task with `options.statusbar.hide` set to `true` stays off the Status Bar.

The default pin limit is three. The maximum supported limit is ten.

An ordinary running task uses `$(sync~spin)`. A background task uses a stable icon and the text status `Online`. The green indicator remains an optional setting.

The extension does not use warning or error backgrounds by default. The user can enable the running highlight, or a task can request one of the two VS Code background theme colors.

The task menu uses the `run-all` Codicon and the label `tasks` by default. The settings panel has an icon picker with previews and a Show label toggle.
...
The settings panel keeps a compact layout: four bar toggles, five small bar fields, one-line task rows, the preview, reset, and the support link. The Menu icon field is a Codicon picker. The Label field has a Show toggle. Task rows expose show, emoji, label, color, and source-edit controls. Task Codicon, tooltip, running overlay, and file-glob metadata remain editable in the task source.
