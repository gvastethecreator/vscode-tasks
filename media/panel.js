(() => {
  ...
  const selectIcon = byId("selectIcon");
  ...
  function iconError(value) {
    return value === "" || iconIds.has(value) ? "" : "Choose an icon from the list.";
  }
  ...
  function renderPreview(state) {
    ...
    if (state.compact) {
      preview.append(text("span", state.selectIcon ? "$(" + state.selectIcon + ")" : "$(run-all)", "pill"));
      return;
    }
  ...
  selectIcon.addEventListener("change", () => {
    const value = selectIcon.value.trim();
    selectIcon.setCustomValidity(iconError(value));
    if (selectIcon.reportValidity()) send({ type: "setSelectIcon", icon: value });
  });
