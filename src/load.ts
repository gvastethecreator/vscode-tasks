...
import { DEFAULT_MENU_ICON, DEFAULT_MENU_LABEL } from "./overflowButton.ts";
...
export function readSettings(): StatusBarSettings {
  ...
  return {
    defaults: {
      hide: booleanSetting(cfg.get<unknown>("default.hide"), false),
      color: parseColor(cfg.get<unknown>("default.color"))?.value ?? "",
    },
    limit,
    compact: booleanSetting(cfg.get<unknown>("compact"), false),
    select: {
      label: boundedSettingText(cfg.get<unknown>("select.label"), 40) || DEFAULT_MENU_LABEL,
      color: parseColor(cfg.get<unknown>("select.color"))?.value ?? "",
      icon: parseCodiconId(cfg.get<string>("select.icon")) ?? DEFAULT_MENU_ICON,
      showLabel: booleanSetting(cfg.get<unknown>("select.showLabel"), true),
    },
    running: {
...
function emptyPanelState(): PanelState {
  return {
    ...
    limit: DEFAULT_LIMIT,
    compact: false,
    selectLabel: DEFAULT_MENU_LABEL,
    selectShowLabel: true,
    selectColor: "",
    selectIcon: DEFAULT_MENU_ICON,
...
export async function buildSnapshot(): Promise<LoadSnapshot> {
  ...
  const panel: PanelState = {
    ...
    selectShowLabel: settings.select.showLabel,
