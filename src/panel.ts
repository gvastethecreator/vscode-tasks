...
import { menuIconMarkup } from "./menuIconSvg.ts";
  ...
  <main class="page">
        ...
        <div class="fields">
          ...
          <div class="field">
            ...
            <div class="icon-picker">
              <button type="button" class="icon-trigger" id="pickIcon" aria-label="Menu icon" aria-haspopup="listbox" aria-expanded="false" aria-controls="iconMenu">
                <span id="pickIconGlyph" aria-hidden="true"></span>
                <span id="pickIconId"></span>
              </button>
              <div class="icon-menu" id="iconMenu" hidden role="listbox" aria-label="Menu icons">${iconOptionsHtml()}</div>
            </div>
          </div>
          <div class="field">
            <span>Label</span>
            <span class="label-row">
              <label class="option"><input id="showSelectLabel" type="checkbox"> Show</label>
              <input id="selectLabel" type="text" maxlength="40" spellcheck="false" aria-label="Menu label">
            </span>
          </div>
          <label class="field">
            <span>Color</span>
          ...
          <label class="field">
            ...
            <span class="color-row"><input id="selectPicker" type="color" aria-label="Label color"><input id="selectColor" type="text" maxlength="80" spellcheck="false" placeholder="#22C1D6"></span>
          </label>
        </div>
      </div>
    </section>
    ...
    <div class="footer">
      <div class="footer-actions">
        ...
        <button type="button" class="cmd secondary" id="setDefaults">Set defaults</button>
      </div>
      <p class="support">
...
function iconOptionsHtml(): string {
  return TASK_CODICONS.map((id) => {
    return `<button type="button" class="icon-option" role="option" data-icon="${id}" title="${id}" aria-label="${id}" aria-selected="false">${menuIconMarkup(id)}</button>`;
  }).join("");
}
